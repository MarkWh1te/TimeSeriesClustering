...
func get_centroid(datas map[string][]float64, n int) ([][]float64, map[int][]int, []string, map[string][]float64) {
	...
	centroids, assignments, sumdistance := k_means_clust(data_list, n, 20, 3)
...
func k_means_clust(data_list map[int][]float64, num_clust int, num_iter int, w int) ([][]float64, map[int][]int, map[int]float64) {
...
func get_mean(data_list [][]float64) []float64 {
	var clust_sum []float64
	for _, vv := range data_list {
...
func main() {
	xxx := make(map[int][]float64)
	xxx [0] = []float64{1, 2, 3, 4, 5}
	xxx [1] = []float64{11, 2, 13, 4, 5}
	xxx [2] = []float64{12, 22, 32, 42, 5}
	xxx [3] = []float64{22, 22, 32, 42, 51}
	xxx [4] = []float64{12, 22, 32, 42, 35}
	xxx [5] = []float64{11, 12, 13, 14, 5}
	xxx [6] = []float64{12, 22, 32, 42, 25}
	xxx [7] = []float64{16, 26, 36, 4, 5}
	xxx [8] = []float64{17, 23, 30, 41, 15}
	xxx [9] = []float64{19, 26, 33, 45, 52}
	xxx [10] = []float64{12, 12, 32, 42, 5}
	xxx [11] = []float64{11, 21, 31, 41, 51}
	xxx [12] = []float64{17, 23, 30, 41, 5}
	fmt.Println(bisecting_k_means_clust(xxx, 3, 100, 5))
}
...
func bisecting_k_means_clust(data_list map[int][]float64, num_clust int, num_iter int, w int) ([][]float64, [][]int) {
	...
	for {
		...
		for i := 0; i < 100; i++ {
			...
			//fmt.Println("zzz")
			centroids, assignments, sumdistance := k_means_clust(data_list, 2, num_iter, w)
			//fmt.Println("iii")
